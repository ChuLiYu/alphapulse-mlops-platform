...
export interface TradingSignal {
  ...
  confidence: number; price: number; timestamp: number;
  model: string; pnl_pct: number;
}

export interface PipelineRun {
  dag_id: string; run_id: string;
  state: 'running' | 'success' | 'failed' | 'queued';
  start_time: number; duration_sec: number;
  tasks_done: number; tasks_total: number;
}

export interface ModelMetric {
  name: string; version: string;
  accuracy: number; sharpe: number; max_drawdown: number;
...
const SEED_PRICES: Record<string, number> = {
  'BTC-USD': 67420, 'ETH-USD': 3540, 'SPY': 528, 'QQQ': 452,
};
...
class MockDataEngine {
  ...
  getPortfolio = ()                  => 125000 + Math.sin(this.tick * 0.08) * 3200 + gaussianRandom() * 800;
  ...
  getModels(): ModelMetric[] {
    ...
    return [
      { name: 'CatBoost', version: 'v3.2', accuracy: b+0.04, sharpe: 1.82, max_drawdown: -0.087, status: 'production' },
      { name: 'XGBoost',  version: 'v2.1', accuracy: b+0.01, sharpe: 1.61, max_drawdown: -0.112, status: 'staging'    },
      { name: 'LightGBM', version: 'v4.0', accuracy: b-0.01, sharpe: 1.74, max_drawdown: -0.094, status: 'staging'    },
      { name: 'Ensemble', version: 'v1.0', accuracy: b+0.06, sharpe: 1.95, max_drawdown: -0.071, status: 'production' },
    ];
  }
    ...
    this.pipelineStates = this.pipelineStates.map(p => {
      if (p.state === 'running') {
        ...
        return done >= p.tasks_total
          ? { ...p, state: 'success' as const, tasks_done: p.tasks_total, duration_sec: Math.floor(Math.random()*300+100) }
          : { ...p, tasks_done: done };
      }
      if (p.state === 'success' && Math.random() < 0.15)
        return { ...p, state: 'queued' as const, tasks_done: 0, run_id: `run_${Date.now()}` };
...
export const usePrices    = () => useMockData(() => ({
...
export const useHistory   = (sym: string, n?: number) => useMockData(() => mockEngine.getHistory(sym, n));
...
### Dashboard.tsx
```typescript
import { usePrices, useHistory, useSignals, usePortfolio } from '../mock/useMockData';
  ...
  return (
    <div style={{
      ...
      <span>
        ...
        {' '}— Prices simulated via Geometric Brownian Motion. No real trades. No backend.
      </span>
      <a href="https://github.com/ChuLiYu/alphapulse-mlops-platform"
...
# Part 7：Git Branch 策略 — 雙軌 Portfolio

> **目的：** 同一個 repo，DE 找工作用 `main`，未來找 MLOps 用 `feature/mlops-full-stack`。
> 服務不丟，定位清晰，面試時展示架構取捨能力。
...
```
main
│  定位：WealthTech Data Engineer Portfolio
│  服務：frontend · postgres · minio · airflow
│  資源：輕量，OCI Free 不發熱
│
├── feature/hero-demo
│      定位：純靜態 Demo，後端全 scale-to-0
│
└── feature/mlops-full-stack
       定位：完整 MLOps Platform，未來找 MLOps 職缺時切換
       服務：+ ollama · trainer · mlflow · grafana · fastapi
       資源：需要完整 OCI 24GB RAM
```

---

## Task 25｜建立 `feature/mlops-full-stack` 分支

**在做任何 main 的清理之前，先執行以下操作：**

```bash
# 從現在的完整狀態建立 mlops 分支，保留所有服務
git checkout -b feature/mlops-full-stack
git push origin feature/mlops-full-stack
...
## Task 26｜清理 `main` 的 docker-compose.yml

**目標檔案**：`infra/docker-compose.yml`（或專案根目錄的 `docker-compose.yml`）

從 `main` 分支的 docker-compose 中**完整移除**以下 service 區塊：

| 移除的 Service | 原因 |
|---|---|
| `ollama` | LLM 推理，吃資源最兇，DE 職缺不需要展示 |
| `trainer` | 即時訓練，Demo 用不到 |
| `grafana` | Demo 無真實 metrics，移除減輕負擔 |
| `fastapi` | Demo Mode 前端不 call API |
| `mc` | MinIO Client，只是管理工具，不是核心服務 |
| `airflow-init` | 初始化完成後不需要持續運行 |

**保留在 `main` 的 services：**

```yaml
services:
  postgres:      # Airflow metadata + 數據倉儲
  minio:         # Bronze layer 儲存
  airflow-webserver:
  airflow-scheduler:
  airflow-worker:
  frontend:      # React Demo
...
**移除方式**：直接刪除對應的 service 區塊，包含其 `depends_on`、`volumes`、`networks` 引用。

---

## Task 27｜清理 `main` 的 K3s Manifests

**目標目錄**：`infra/k3s/base/`

將以下 Deployment manifests 從 `main` 分支**移至** `feature/mlops-full-stack`，或直接在 `main` 設定 `replicas: 0`：

```
infra/k3s/base/ollama-deployment.yaml      → 移除或 replicas: 0
infra/k3s/base/trainer-deployment.yaml     → 移除或 replicas: 0
infra/k3s/base/grafana-deployment.yaml     → 移除或 replicas: 0
infra/k3s/base/fastapi-deployment.yaml     → 移除或 replicas: 0
```

**建議做法**：用 `replicas: 0` 而非直接刪除，這樣 YAML 定義保留在 `main`，
面試官 review 程式碼時仍能看到你有這些服務的設定能力。

---

## Task 28｜更新兩個分支的 README

### `main` 分支 README 加入分支說明

在 Role-Specific Navigation 表格之後加入：

```markdown
## 🌿 Branch Strategy

This repository maintains two purpose-built branches:

| Branch | Purpose | Services Running |
|--------|---------|-----------------|
| `main` | **WealthTech DE Portfolio** — lightweight data pipeline focus | Airflow · PostgreSQL · MinIO · Frontend |
| `feature/mlops-full-stack` | **Full MLOps Platform** — complete ML infrastructure | + Ollama · MLflow · Trainer · Grafana · FastAPI |

> The `feature/mlops-full-stack` branch demonstrates the full MLOps capability
> including LLM inference (Ollama), real-time model training, and drift monitoring.
> Switch branches based on the role you're hiring for.
```

### `feature/mlops-full-stack` 分支 README 頂部加入 Banner

```markdown
> ⚠️ **This is the `feature/mlops-full-stack` branch.**
> Full MLOps stack including Ollama, MLflow, Trainer, and Grafana.
> Requires 24GB RAM. For the lightweight DE-focused version, see [`main`](../../tree/main).
```

---

## Task 29｜更新 GitHub Actions — 分支對應 Workflow

**原則：** 不同分支觸發不同的 deploy workflow。

在現有 workflows 加入 branch filter：

**`.github/workflows/deploy-demo.yml`**（只跑 `main`）：
```yaml
on:
  push:
    branches: [main]          # ← 只有 main 觸發
    paths: ['frontend/**', 'infra/k3s/overlays/demo/**']
```

**`.github/workflows/deploy-mlops.yml`**（新建，只跑 mlops 分支）：
```yaml
name: Deploy Full MLOps Stack

on:
  push:
    branches: [feature/mlops-full-stack]   # ← 只有這個分支觸發
  workflow_dispatch:
...
jobs:
  deploy-full:
    ...
    steps:
      ...
      - name: Deploy Full Stack to OCI K3s
        uses: appleboy/ssh-action@master
        with:
          host: ${{ secrets.OCI_HOST }}
          username: ${{ secrets.OCI_USER }}
          key: ${{ secrets.OCI_SSH_KEY }}
          script: |
            # Apply base overlay — all services including MLOps
            kubectl apply -k ~/alphapulse/infra/k3s/base/

            # Scale up all services
            kubectl scale deployment ollama trainer mlflow grafana fastapi \
              --replicas=1 -n application

            echo "✅ Full MLOps stack deployed."
```
...
- [ ] `feature/mlops-full-stack` 分支已建立並 push，包含完整服務設定
- [ ] `main` 分支的 docker-compose 已移除 ollama / trainer / grafana / fastapi / mc
- [ ] `main` 分支 K3s manifests 中上述服務已設 `replicas: 0`
- [ ] 兩個分支的 README 都有分支說明
- [ ] `deploy-demo.yml` 只在 `main` 觸發
- [ ] `deploy-mlops.yml` 只在 `feature/mlops-full-stack` 觸發
- [ ] 本機電腦發熱問題解決（移除 ollama 後應立即改善）

---

## 面試時怎麼說這個設計

> "I deliberately separated the DE-focused pipeline from the full MLOps stack
> into two branches. The `main` branch demonstrates what matters for a Data Engineer
> role — Airflow orchestration, dbt Medallion Architecture, and financial data
> precision. The MLOps branch retains the full infrastructure including LLM inference
> and real-time model training, which I can switch to when discussing MLOps roles.
> This separation also reflects a real architectural decision — not every production
> system needs all services running simultaneously."

這段話展示的是**刻意的架構取捨**，不是「我不會」，而是「我知道什麼時候該關什麼」。

---

*文檔版本：v2.1 — 新增 Part 7 Git Branch 策略*
