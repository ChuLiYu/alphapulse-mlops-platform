...
class FeatureProducer:
    ...
    @staticmethod
    def produce_stationary_features(price_df: pd.DataFrame, sentiment_df: pd.DataFrame) -> pd.DataFrame:
        # --- 1. Alignment & Gap Filling ---
        price_df['date'] = pd.to_datetime(price_df['date'])
        price_df = price_df.groupby(pd.Grouper(key='date', freq='1h')).first()
        full_range = pd.date_range(start=price_df.index.min(), end=price_df.index.max(), freq='1h')
        ...
        for w in [1, 2, 3, 6, 12, 24, 48, 72, 168]:
            df[f'feat_ret_log_{w}h'] = np.log(df['close'] / df['close'].shift(w)).fillna(0)
            
        # B. Volatility (Multi-window std of returns)
        log_ret = np.log(df['close'] / df['close'].shift(1))
        for w in [6, 12, 24, 48, 168]:
            df[f'feat_volatility_{w}h'] = log_ret.rolling(w, min_periods=1).std().fillna(0)
        ...
        for w in [6, 12, 24, 48, 72, 168]:
            df[f'feat_sent_ma_{w}h'] = df['sentiment_avg'].rolling(w, min_periods=1).mean().fillna(0)
        ...
        final_cols = ['date', 'ticker', 'target_return', 'metadata_close'] + feature_cols
