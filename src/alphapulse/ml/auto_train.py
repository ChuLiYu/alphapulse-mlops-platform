...
from alphapulse.ml.validation import WalkForwardCV, WalkForwardResult, OutOfSampleValidator
...
DEFAULT_MODELS = [
    # === XGBoost Variants ===
    ModelConfig("xgboost_regularized", xgb.XGBRegressor, {
        "n_estimators": 100, "max_depth": 3, "learning_rate": 0.05,
        "reg_alpha": 0.1, "reg_lambda": 1.0  # L1 + L2 regularization
    }),
    ModelConfig("xgboost_shallow", xgb.XGBRegressor, {
        "n_estimators": 100, "max_depth": 3, "learning_rate": 0.05,
        "reg_alpha": 0.05, "reg_lambda": 0.5
    }),
    ModelConfig("xgboost_deep", xgb.XGBRegressor, {
        "n_estimators": 150, "max_depth": 5, "learning_rate": 0.1,
        "reg_alpha": 0.1, "reg_lambda": 1.0
    }),
    ModelConfig("xgboost_conservative", xgb.XGBRegressor, {
        "n_estimators": 50, "max_depth": 2, "learning_rate": 0.03,
        "reg_alpha": 0.2, "reg_lambda": 2.0  # Stronger regularization
    }),
    # NEW: Heavy regularization for overfitting prevention
    ModelConfig("xgboost_heavy_reg", xgb.XGBRegressor, {
        "n_estimators": 80, "max_depth": 2, "learning_rate": 0.01,
        "reg_alpha": 0.5, "reg_lambda": 3.0, "subsample": 0.8, "colsample_bytree": 0.8
    }),
    # NEW: Moderate depth with dropout-like behavior
    ModelConfig("xgboost_subsample", xgb.XGBRegressor, {
        "n_estimators": 120, "max_depth": 4, "learning_rate": 0.05,
        "reg_alpha": 0.15, "reg_lambda": 1.5, "subsample": 0.7, "colsample_bytree": 0.7
    }),
    # === Traditional ML ===
    ModelConfig("random_forest", RandomForestRegressor, {
        "n_estimators": 100, "max_depth": 5, "n_jobs": -1, "min_samples_leaf": 5
    }),
    ModelConfig("random_forest_deep", RandomForestRegressor, {
        "n_estimators": 150, "max_depth": 8, "n_jobs": -1, "min_samples_leaf": 3
    }),
    ModelConfig("gradient_boosting", GradientBoostingRegressor, {
        "n_estimators": 100, "max_depth": 3, "learning_rate": 0.1
    }),
    ModelConfig("gradient_boosting_conservative", GradientBoostingRegressor, {
...
if LIGHTGBM_AVAILABLE:
    DEFAULT_MODELS.extend([
        ModelConfig("lightgbm_default", lgb.LGBMRegressor, {
            "n_estimators": 100, "max_depth": 4, "learning_rate": 0.05,
            "reg_alpha": 0.1, "reg_lambda": 0.1, "verbose": -1
        }),
        ModelConfig("lightgbm_conservative", lgb.LGBMRegressor, {
            "n_estimators": 80, "max_depth": 3, "learning_rate": 0.03,
            "reg_alpha": 0.2, "reg_lambda": 0.5, "verbose": -1
        }),
    ])


# Expanded thresholds to test (finer-grained search)
DEFAULT_THRESHOLDS = [0.0005, 0.001, 0.0015, 0.002, 0.0025, 0.003, 0.004, 0.005, 0.007, 0.01]
...
class AutoTrainer:
    ...
    def load_data(self):
        ...
        print(f"Data loaded: Train={len(self.train)}, Val={len(self.val)}, Test={len(self.test)}")
    ...
    def run(
        ...
        print(f"Evaluation mode: {'Walk-Forward CV' if use_walk_forward else 'Single Backtest'}")
        print(f"{'='*60}\n")
        ...
        for model_config in models:
            for threshold in thresholds:
                try:
                    if use_walk_forward:
                        result = self.walk_forward_evaluate(model_config, threshold, n_splits=wf_splits)
                    ...
                    if use_walk_forward:
                        ...
                        print(f"{model_config.name:20} | thr={threshold:.4f} | Sharpe={sharpe:6.3f}±{std_sharpe:.3f} | Ret={ret:6.2f}% | Trades={trades}")
                    else:
                        print(f"{model_config.name:20} | thr={threshold:.4f} | Sharpe={sharpe:6.3f} | Ret={ret:6.2f}% | Trades={trades}")
        ...
        try:
            ...
            with mlflow.start_run(run_name=run_name):
                ...
                mlflow.log_params({
                    ...
                    "evaluation_mode": "walk_forward" if use_walk_forward else "single_backtest",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", default="/app/data/processed",
                        help="Directory with train/val/test parquet files")
    parser.add_argument("--output_dir", default="/app/models/saved",
                        help="Directory to save best model")
    parser.add_argument("--mlflow_uri", default="http://mlflow:5000",
