...
def generate_report(
    ...
    report.append("|--------|-------|--------|--------|")
    report.append(
        f"| Total Return | {total_return:.2f}% | > 0% | {'✅' if total_return > 0 else '❌'} |"
    )
    report.append(
        f"| Sharpe Ratio | {sharpe:.4f} | > 1.0 | {'✅' if sharpe > 1.0 else '⚠️' if sharpe > 0 else '❌'} |"
    )
    report.append(
        f"| Max Drawdown | {max_dd:.2f}% | < 20% | {'✅' if max_dd < 20 else '⚠️' if max_dd < 30 else '❌'} |"
    )
    report.append(
        f"| Win Rate | {win_rate:.2f}% | > 50% | {'✅' if win_rate > 50 else '⚠️'} |"
    )
    report.append("")
    ...
    if training_metrics:
        ...
        report.append(
            "> Training metrics not available. Check MLflow UI for experiment details."
        )
    report.append("")
    ...
    report.append(
        f"- **Final Portfolio**: ${backtest.get('final_portfolio_value', 'N/A')}"
    )
    ...
    report.append(
        f"- **Daily Returns Tracked**: {backtest.get('num_daily_returns', 'N/A')}"
    )
    report.append("")
    ...
    if sharpe < 1.0:
        recommendations.append(
            "- Consider feature engineering or hyperparameter tuning to improve Sharpe Ratio"
        )
    if max_dd > 20:
        recommendations.append(
            "- Implement stop-loss mechanisms to reduce maximum drawdown"
        )
    if win_rate < 50:
        recommendations.append("- Review signal threshold to improve trade quality")
    if len(recommendations) == 0:
        recommendations.append(
            "- Model meets production criteria. Consider A/B testing in staging environment."
        )
...
def main():
    ...
    parser.add_argument(
        "--model_id", type=str, default="btc_predictor_v1", help="Model identifier"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
