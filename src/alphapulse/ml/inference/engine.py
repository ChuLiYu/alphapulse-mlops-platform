...
class InferenceEngine:
    ...
    def load_model_by_stage(self, stage: str):
        ...
        try:
            ...
            import glob
            search_dir = "/app/models/production" if stage == "Production" else "/app/models/saved"
            available = glob.glob(f"{search_dir}/*.pkl")
            if available:
                latest = max(available, key=os.path.getmtime)
                import joblib
                model = joblib.load(latest)
                logger.info(f"Fallback: Loaded latest available {stage} model from {latest}")
                return model
            return None
    ...
    def generate_signals(self):
        ...
        results = []
        current_price = float(df['metadata_close'].iloc[0]) if 'metadata_close' in df.columns else 0.0
        
        # 1. Champion Inference (The real deal)
        if champion:
            pred = champion.predict(X)[0]
            self._save_signal(pred, "Champion_Prod", is_shadow=False, price=current_price)
            results.append("Champion_Active")

        # 2. Challenger Inference (Shadow testing)
        if challenger:
            pred = challenger.predict(X)[0]
            self._save_signal(pred, "Challenger_Staging", is_shadow=True, price=current_price)
    ...
    def _save_signal(self, prediction, model_ver, is_shadow, price):
        ...
        signal_type = "HOLD"
        if prediction > 0.005: signal_type = "BUY"
        elif prediction < -0.005: signal_type = "SELL"
