...
class Backtester:
    ...
    def run(self, test_data: pd.DataFrame, target_col: str = "target") -> BacktestResult:
        """
        ...
        for i in range(len(test_data)):
            ...
            if signal == Signal.BUY and position != Signal.BUY and cash > Decimal("0"):
            ...
            elif signal == Signal.SELL and position == Signal.BUY and shares > Decimal("0"):
                ...
                if len(self.trades) > 0:
                    last_buy = next((t for t in reversed(self.trades) if t.signal == Signal.BUY), None)
                    if last_buy:
        ...
        # Final calculations
        final_value = self.portfolio_values[-1] if self.portfolio_values else self.config.initial_capital
