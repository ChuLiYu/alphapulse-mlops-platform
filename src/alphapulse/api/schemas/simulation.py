...
class SimulationRequest(BaseModel):
    ...
    confidence_threshold: float = Field(..., ge=0.5, le=0.99, description="Minimum model confidence to trade")
    stop_loss_pct: float = Field(..., ge=0.01, le=0.10, description="Stop loss percentage (0.01 - 0.10)")
    initial_capital: float = Field(..., gt=0, description="Initial capital for the simulation")
