...
def backfill_btc_prices():
    ...
    btc = yf.download("BTC-USD", start="2016-01-01", end=datetime.now().strftime("%Y-%m-%d"))
    ...
    df['log_return'] = df['daily_return'].apply(lambda x: 0 if x <= -1 else (0 if pd.isna(x) else (float(x) if x == 0 else (pd.NA if x < -1 else (pd.NA))))) # Dummy for now
    ...
    print(f"🎉 Successfully backfilled {len(df)} days of real BTC price data into the system!")
