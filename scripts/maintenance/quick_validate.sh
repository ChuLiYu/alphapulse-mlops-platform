...
echo "------------------"
check_service "PostgreSQL" "docker ps | grep alphapulse-postgres | grep -q healthy"
check_service "FastAPI" "docker ps | grep alphapulse-fastapi | grep -q healthy"
check_service "Airflow" "docker ps | grep alphapulse-airflow-webserver"
check_service "MLflow" "docker ps | grep alphapulse-mlflow"
check_service "MinIO" "docker ps | grep alphapulse-minio | grep -q healthy"

echo ""
echo "📊 Database Tables"
echo "------------------"
check_service "prices table" "docker exec alphapulse-postgres psql -U postgres -d alphapulse -c 'SELECT 1 FROM prices LIMIT 1'"
check_service "technical_indicators table" "docker exec alphapulse-postgres psql -U postgres -d alphapulse -c 'SELECT 1 FROM technical_indicators LIMIT 1'"
check_service "sentiment_scores table" "docker exec alphapulse-postgres psql -U postgres -d alphapulse -c 'SELECT 1 FROM sentiment_scores LIMIT 1' 2>/dev/null || true"
