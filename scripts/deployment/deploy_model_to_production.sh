...
if docker exec trainer test -f /app/models/saved/best_model.pkl; then
...
SUMMARY=$(docker exec trainer cat /app/models/saved/training_summary.json 2>/dev/null || echo "{}")

if [ "$SUMMARY" != "{}" ]; then
    TEST_MAE=$(echo "$SUMMARY" | docker exec -i trainer python3 -c "import sys, json; data=json.load(sys.stdin); print(data.get('best_model', {}).get('test_mae', 0))")
    TEST_R2=$(echo "$SUMMARY" | docker exec -i trainer python3 -c "import sys, json; data=json.load(sys.stdin); print(data.get('best_model', {}).get('test_r2', 0))")
    ...
    MEETS_MAE=$(docker exec trainer python3 -c "print('yes' if $TEST_MAE < $MAE_THRESHOLD else 'no')")
    MEETS_R2=$(docker exec trainer python3 -c "print('yes' if $TEST_R2 > $R2_THRESHOLD else 'no')")
...
docker exec trainer mkdir -p /app/models/production
docker exec trainer cp /app/models/saved/best_model.pkl /app/models/production/
docker exec trainer cp /app/models/saved/training_summary.json /app/models/production/ 2>/dev/null || true

# Also copy to FastAPI container if needed
docker cp trainer:/app/models/saved/best_model.pkl /tmp/best_model.pkl
docker cp /tmp/best_model.pkl fastapi:/app/models/production/ 2>/dev/null || true
...
echo "$DEPLOYMENT_INFO" | docker exec -i trainer tee /app/models/production/deployment_info.json > /dev/null
...
docker exec trainer cat /app/models/production/deployment_info.json | docker exec -i trainer python3 -c "import sys, json; data=json.load(sys.stdin); print(f\"  Deployed: {data['deployment_time']}\"); print(f\"  Test MAE: {data['performance']['test_mae']:.6f}\"); print(f\"  Test R²: {data['performance']['test_r2']:.4f}\")"
echo ""
echo "🌐 API Endpoints:"
