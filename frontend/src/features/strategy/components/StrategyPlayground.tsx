...
const StrategyPlayground: React.FC = () => {
  ...
  const runSimulation = async () => {
    ...
    try {
      ...
      const mockData = generateSimulationData(riskLevel, confidence, stopLoss);
      setSimulationData(mockData);
      // Calculate mock metrics
      const total_return =
        ((mockData[mockData.length - 1].value - 10000) / 10000) * 100;
      setMetrics({ total_return });
    } finally {
      setLoading(false);
    }
  };
      ...
      <Card sx={{ mt: 4, overflow: "hidden" }}>
        <Box
          sx={{
            ...
            borderColor: "divider",
            bgcolor: alpha("#00d2ff", 0.03),
          }}
        >
          <Stack direction="row" alignItems="center" spacing={1}>
            <AutoGraphIcon color="primary" />
            <Typography variant="h6">Strategy Playground</Typography>
          </Stack>
          <Typography variant="body2" color="text.secondary">
            Simulate AlphaPulse model performance with different risk
            configurations.
          </Typography>
        </Box>
          ...
          <Grid container>
            ...
            >
              ...
              <Box sx={{ p: 3, textAlign: "center" }}>
                ...
                {/* Rerun button removed. Projected return updates automatically when parameters change. */}
              </Box>
            </Grid>
            ...
            <Grid
              ...
              xs={12}
              md={8}
              sx={{ bgcolor: alpha("#000", 0.2), p: 2, position: "relative" }}
            >
              {loading && (
                <Box
                  sx={{
              ...
              <StrategyChart
                data={simulationData.length > 0 ? simulationData : []}
              />
            </Grid>
          </Grid>
        </CardContent>
