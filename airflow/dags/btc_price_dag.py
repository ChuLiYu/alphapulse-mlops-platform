...
def btc_price_pipeline():
    ...
    def calculate_indicators_task(data):
        ...
        for col in df_indicators.select_dtypes(include=['datetime', 'datetimetz']).columns:
    ...
    def save_to_minio_task(data):
        df = pd.DataFrame(data)
        object_name = f'btc_price_indicators_{pd.Timestamp.now().strftime("%Y%m%d%H%M%S")}.csv'
